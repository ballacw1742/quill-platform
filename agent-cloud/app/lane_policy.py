"""Lane-decision policy for cloud-agent proposals (Phase 1, GAP §9.4).

Replaces the hard-coded ``lane=2`` in ``app.approvals.create_proposal`` with a
risk-graded decision consulting:

  * the agent's trust tier (``AgentRegistration.trust_tier``, api-side),
...
but this module is vendored into agent-cloud because the proposal path runs
in-process there and agent-cloud cannot import the runtime package at runtime
(separate Docker build context). The api approvals executor re-validates
independently (belt #2); this is belt #1.
...
_CONTRACT_PATH = Path(__file__).with_name("contracts") / "lane_decision.json"
