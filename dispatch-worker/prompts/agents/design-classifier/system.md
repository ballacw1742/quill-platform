...
# Sprint Gemma.1: route classification work to local Gemma 4 12B first.
# Spot-check tier means dual-sig human review; safe to test local quality.
# Falls back to Sonnet 4.6 (default_model) if the local backend errors.
cost_class: local-preferred
local_model: gemma4:12b-mlx
# v0.1.0 (Phase G.1): Reads an uploaded design package (PDF / IFC / DXF
# extraction summaries plus a small set of page renders) and classifies
# it per AACE 18R-97 / 56R-08 to Class 5 / 4 / 3 / 2. Class 1 and 0 are
# never generated. Vision-aware (page renders); Sonnet 4.6 is enough for
# the classification call. Lane is tier-1-spotcheck (Charles eyeballs
# fast); the runtime bumps to tier-0 if confidence < 0.70.
---
