...
output_schema: schemas/contract_extraction.schema.json
trust_tier_default: tier-1-spotcheck
description: >
  Reads extracted plain text from a construction contract document and emits
  structured fields (contract_type, parties, dates, monetary values, obligations,
  and notable clauses). Extraction only — no review, no legal opinions.
tags:
  - contracts
  - extraction
  - construction
