...
- `Gemma 4 12B cold-start latency` — First call after model load takes ~15-20s. Subsequent calls are fast (~2-4s). — `(visible-tolerable)` — **fixed in Gemma.2** via `quill-runtime local warmup` daemon (60s keep-alive pings).
