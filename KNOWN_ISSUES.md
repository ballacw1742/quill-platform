...
| 14 | After the quillpm.com WebAuthn RP move, old-RP passkeys are orphaned (browsers refuse them on the new domain) but remain listed on `/profile/passkeys` and can't be distinguished from new-RP credentials server-side (`rp_id` isn't stored). Users must re-register; the page shows a re-registration banner when zero active passkeys exist and the approve flow offers password re-auth as fallback. Users can revoke stale entries manually. | visible-but-tolerable | Sprint approvals-password-fallback (2026-07-07) | post-handover (store `rp_id` per credential to auto-flag orphans) |
