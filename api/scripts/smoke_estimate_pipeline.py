...
def build_classifier_input(extraction_results: list[Any]) -> dict[str, Any]:
    ...
    for r in extraction_results:
        entry: dict[str, Any] = {
            ...
            "extraction_summary": r.summary,
        }
        if r.kind == "pdf":
            entry["page_count"] = r.entities.get("page_count", 0)
            # Cap renders to 3 for cost / token control
            entry["renders"] = (r.renders or [])[:3]
            entry["extracted_text_excerpts"] = r.entities.get(
                "text_excerpts", []
            )[:5]
        elif r.kind == "ifc":
            entry["ifc_entities"] = r.entities
            entry["ifc_quantities"] = r.quantities
        files.append(entry)
    return {
