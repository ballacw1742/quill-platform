...
/**
 * /supply-chain — Supply Chain module (Lovable UI port, feat-lovable-ui-port)
 *
 ...
 *   quill-platform-builder/src/routes/supply-chain.tsx
 * Data layer: prod @/lib/api hooks, envelopes adapted per LOVABLE_PORT_CONTRACT.md.
 */
...
import { VendorCard } from "@/components/supply-chain/VendorCard";
import {
  useEquipment,
  useVendors,
...
function statusBadge(s: string): { label: string; cls: string } {
  switch (s) {
    case "not_ordered": return { label: "Not Ordered", cls: "text-label-secondary bg-fill-quaternary" };
    case "ordered":     return { label: "Ordered",     cls: "text-info bg-info/15" };
    case "in_transit":  return { label: "In Transit",  cls: "text-warning bg-warning/15" };
    case "received":    return { label: "Received",    cls: "text-success bg-success/15" };
    case "installed":   return { label: "Installed",   cls: "text-success bg-success/15" };
    case "cancelled":   return { label: "Cancelled",   cls: "text-label-tertiary bg-fill-quaternary" };
    default:            return { label: s,             cls: "text-label-secondary bg-fill-quaternary" };
  }
}
...
function SummaryBar({ summary }: { summary: SupplyChainSummary }) {
  const stats = [
    { label: "Equipment Items", value: String(summary.total_equipment_items) },
    {
      label: "Total Value",
      value:
        summary.total_equipment_value_usd > 0
      ...
      {stats.map((s) => (
        <div
          ...
          className={cn(
            ...
            "bg-chrome border border-hairline",
          )}
        >
          <p
            className={cn(
              "text-title-3 font-bold tabular-nums",
              s.urgent ? "text-danger" : "text-label-primary",
            )}
          >
            {s.value}
          </p>
    ...
    >
      <span
        className={cn(
          ...
          eq.at_risk ? "bg-danger/15 text-danger" : "bg-accent/10 text-accent",
        )}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-caption-1 text-label-secondary truncate">
          {eq.project_id ? `Project ${eq.project_id.slice(0, 10)}…` : "No project"}
...
function AddEquipmentModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = React.useState({
    name: "",
    category: "generator" as EquipmentCategory,
    project_id: "",
    quantity: "1",
    unit_cost_usd: "",
    lead_time_weeks: "",
    ...
    expected_delivery: "",
    vendor_id: "",
    status: "not_ordered" as EquipmentStatus,
    notes: "",
  });
  ...
  const inputCls =
    "w-full rounded-xl bg-bg-elevated border border-hairline px-3 py-2 text-body text-label-primary placeholder:text-label-tertiary focus:outline-none focus:border-accent";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-lg rounded-t-3xl bg-chrome border-t border-hairline pb-safe overflow-y-auto max-h-[90vh]">
        <div className="sticky top-0 flex items-center justify-between px-4 py-4 bg-chrome border-b border-hairline">
          <h2 className="text-title-3 font-bold text-label-primary">
            Add Equipment
          </h2>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-1.5 hover:bg-bg-elevated active:bg-bg-elevated"
          >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-4 py-4">
          <div>
            <label className="text-footnote font-semibold text-label-secondary block mb-1">
              Name *
            </label>
            <input
              required
          ...
          <button
            ...
            className="w-full rounded-full bg-accent py-3 text-body font-semibold text-primary-foreground shadow-card hover:bg-accent-pressed hover:shadow-elevated active:scale-[0.98] transition-all disabled:opacity-50 active:opacity-80"
          >
...
function AddVendorModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = React.useState({
    name: "",
    category: "generator" as VendorCategory,
    contact_name: "",
    contact_email: "",
    contact_phone: "",
    website: "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await createVendor.mutateAsync({
      ...
      performance_score: form.performance_score
        ...
        : null,
      notes: form.notes || null,
    } as Parameters<ReturnType<typeof useCreateVendor>["mutateAsync"]>[0]);
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg rounded-t-3xl bg-chrome border-t border-hairline pb-safe overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={form.category}
                onChange={(e) =>
                  set("category", e.target.value as VendorCategory)
                }
                className={inputCls}
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-end pb-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-body text-label-primary">
                  Pre-qualified
                </span>
              </label>
            </div>
          ...
          {createVendor.error && (
            <p className="text-caption-1 text-danger">
              {(createVendor.error as Error).message}
            </p>
          )}
...
type EquipmentFilter = "all" | "at_risk" | EquipmentStatus;

// ── Main page ─────────────────────────────────────────────────────────────────

export default function SupplyChainPage() {
  const router = useRouter();
  const [activeTab, setActiveTab] = React.useState<"equipment" | "vendors">(
    "equipment",
  );
    ...
    <MobileShell>
      ...
      {activeTab === "equipment" && (
        <>
          ...
          {atRisk.length > 0 && eqFilter !== "at_risk" && (
            <div className="mx-4 mb-4 rounded-2xl bg-warning/10 border border-warning/30 px-4 py-3">
              <div className="flex items-start gap-2">
                <AlertTriangle className="h-4 w-4 text-warning mt-0.5 shrink-0" />
                <div className="flex-1">
                  <p className="text-callout font-semibold text-warning">
                    {atRisk.length} item{atRisk.length !== 1 ? "s" : ""} at
                    delivery risk
                  </p>
                ...
                <button
                  ...
                  className="text-caption-1 font-semibold text-warning hover:text-warning shrink-0"
                >
                  View
                </button>
              </div>
          ...
          <div className="flex flex-wrap gap-2 px-4 mb-3">
            {(
              ...
              ] as { key: EquipmentFilter; label: string }[]
            ).map(({ key, label }) => (
              <button
                key={key}
                type="button"
                onClick={() => setEqFilter(key)}
                className={cn(
                  "rounded-full px-3 py-1 text-caption-1 font-semibold transition-colors",
                  eqFilter === key
                    ? "bg-accent text-primary-foreground"
                    : "bg-bg-elevated text-label-secondary",
                )}
          ...
          {/* Equipment list */}
          <div className="mx-4 rounded-2xl bg-chrome border border-hairline overflow-hidden mb-6">
            {eqLoading ? (
              ...
              equipItems.map((eq) => (
                <EquipmentRow
                  ...
                  eq={eq}
                  onClick={() =>
                    router.push(`/supply-chain/equipment/${eq.id}`)
                  }
                />
              ))
            )}
      ...
      {activeTab === "equipment" && (
        <button
          ...
          className={cn(
            "fixed bottom-[calc(env(safe-area-inset-bottom)+80px)] right-4 z-30",
            "flex h-14 w-14 items-center justify-center",
            "rounded-full bg-accent shadow-card shadow-accent/30",
